...
- [`governance/bundle.py`](governance/bundle.py) — seals the verified ledger + public key into a portable, third-party-verifiable bundle. The manifest (tail hash + event count) is itself **Ed25519-signed**, and the verifier cross-checks it against the re-derived chain — so the bundle is tamper-evident against addition, alteration, reorder, **and truncation** (a public-key-only holder cannot be handed a silently-shortened chain).
